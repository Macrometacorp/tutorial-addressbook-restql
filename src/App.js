...
import { getBaseUrl, makeRegionData } from "./util";
...
class App extends Component {
  ...
  createCollection() {
    const self = this;
    let exist = false;
    let url = `https://api-${this.state.selectedRegionUrl}/_tenant/${
      this.state.tenant
    }/_fabric/${this.state.fabric}/collection`;
    return $.ajax({
      type: "GET",
  ...
  saveQueries() {
    ...
    $.when(readQuery, updateQuery, removeQuery, writeQuery).done(function(
      r1,
      r2,
      r3
    ) {
  ...
  onSavePressed() {
    ...
    if (Object.keys(validationObject).length === 0) {
      ...
      if (isEdit) {
        ...
        const {
          lastEditElem: { _key, firstname, lastname, email },
        } = this.state;
        postData = {
          _key: _key,
          firstName: firstname,
  ...
  renderDialogContent() {
    ...
    let dialogContent = (
      <DialogContent>
        <DialogContentText>
          {this.state.isEdit
            ? "Fill in the fields to be updated."
            : "Provide all the details to add a new contact."}
        </DialogContentText>
        <TextField
  ...
  renderRegionModal() {
    ...
    return (
      <Dialog fullWidth open={regionModal}>
          ...
          <RadioGroup
            onChange={(event) => {
              ...
              const selectedRegionName = availableRegions.find(
                (elem) => elem.value === selectedRegionUrl
              ).label;
      ...
      <div className="App">
        <header className="App-header">
          ...
          <h1 className="App-title">
            Address book is connected to {selectedRegionName}
          </h1>
        </header>
        ...
        <Dialog onClose={this.resetModalData} open={this.state.shouldShowModal}>
          <h1>{this.state.shouldShowModal}</h1>
          <DialogTitle id="form-dialog-title">
            {this.state.isEdit ? "Edit contact details" : "Add contact details"}
          </DialogTitle>
        ...
        {this.state.isLoading && (
          <img src={spinner} alt="logo" className="spinner" />
        )}
      </div>
    );
  }
